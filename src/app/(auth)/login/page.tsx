...
import { signIn, signInWithGoogle, demoMode } from "@/lib/auth";
...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.ok) {
      ...
      setAttempts(count);
      toast("error", count >= 5 ? "Account temporarily locked" : "Login failed", result.error);
      return;
    }
    toast("success", "Welcome back!");
    router.push(result.redirect ?? (next ?? "/dashboard"));
    router.refresh();
  };
  ...
  const handleGoogle = async () => {
    ...
    if (!res.ok) {
      toast("error", "Google login failed", res.error);
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-4">
        <Button variant="outline" className="w-full" onClick={handleGoogle} loading={loading}>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <Field label="Password" error={errors.password} htmlFor="password">
          <div className="relative">
            ...
            <Input id="password" type={show ? "text" : "password"} className="pl-9 pr-10" placeholder="••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} autoComplete="current-password" />
        ...
        <div className="flex items-center justify-between text-sm">
          <label className="flex items-center gap-2 text-muted-foreground">
            <input type="checkbox" defaultChecked className="h-4 w-4 rounded border-border accent-brand-600" />
            Remember me
          </label>
      ...
      {attempts > 0 && (
        <p className="mt-3 flex items-center gap-1.5 text-xs text-warning">
          <ShieldCheck className="h-3.5 w-3.5" /> {5 - attempts} attempts left before your account locks temporarily.
        </p>
      )}

      <p className="mt-6 text-center text-sm text-muted-foreground">
        Don&apos;t have an account?{" "}
        <Link href="/register" className="font-medium text-brand-500 hover:underline">Create one</Link>
      </p>
      ...
      {demoMode() && (
        <div className="mt-5 rounded-xl border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
          ...
          <p className="mt-1">Try: student@akradhii.com · admin@akradhii.com · mentor@akradhii.com · client@akradhii.com · employee@akradhii.com — password <code className="rounded bg-muted px-1">Akradhii@123</code></p>
        </div>
      )}
    </div>
  );
